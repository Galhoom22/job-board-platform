    ...
    <div class="overflow-x-auto p-6">
        <div class="max-w-2xl mx-auto mt-6">
            ...
            <form action="{{ route('job-categories.update', $category->id) }}" method="POST"
                class="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 relative overflow-hidden">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <p class="text-sm text-gray-500 mt-1">{{ __('Update the classification for job listings.') }}
                    </p>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}"
                            class="w-full pl-10 pr-4 py-3 rounded-lg border
                                    ...
                                    focus:ring-4 focus:ring-opacity-50 transition-all shadow-sm"
                    ...
                    <textarea id="description" name="description" rows="5"
                        class="w-full px-4 py-3 rounded-lg border
                                {{ $errors->has('description')
                                    ? 'border-red-300 focus:border-red-500 focus:ring-red-200'
                                    : 'border-gray-300 focus:border-blue-500 focus:ring-blue-200' }}
                                focus:ring-4 focus:ring-opacity-50 transition-all shadow-sm resize-none"
                        placeholder="{{ __('Briefly describe the responsibilities or scope...') }}">{{ old('description', $category->description) }}</textarea>
